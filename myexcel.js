...
(function () {
    ...
    var templateDrawing = '<xdr:twoCellAnchor editAs="oneCell">' +
        ...
        '<xdr:to><xdr:col>{colTo}</xdr:col><xdr:colOff>695325</xdr:colOff><xdr:row>{rowTo}</xdr:row><xdr:rowOff>123825</xdr:rowOff></xdr:to>' +
        '<xdr:pic><xdr:nvPicPr><xdr:cNvPr id="{id}" name="Image {imgid}"><a:extLst><a:ext uri="{FF2B5EF4-FFF2-40B4-BE49-F238E27FC236}">' +
        '<a16:creationId xmlns:a16="http://schemas.microsoft.com/office/drawing/2014/main" id="{{creationId}}"/>' +
        '</a:ext></a:extLst></xdr:cNvPr><xdr:cNvPicPr><a:picLocks noChangeAspect="1"/></xdr:cNvPicPr></xdr:nvPicPr><xdr:blipFill><a:blip xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" r:embed="rId{rid}"><a:extLst><a:ext uri="{28A0092B-C50C-407E-A947-70E740481C1C}"><a14:useLocalDpi xmlns:a14="http://schemas.microsoft.com/office/drawing/2010/main" val="0"/></a:ext></a:extLst></a:blip><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:xfrm><a:off x="1524000" y="190500"/><a:ext cx="5267325" cy="695325"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr></xdr:pic>' +
        '<xdr:clientData/>' +
        '</xdr:twoCellAnchor>';
    ...
    function getSheetRelsAsXml(sheet, index) {
        var d = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
                '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">';
        for (let i = 0; i < sheet.media.length; i++) {
            d += '<Relationship Id="rId'+(i+1)+'" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" Target="../drawings/drawing'+index+'.xml"/>'; 
        }
        d += '</Relationships>';
        return d;
    }

    function getDrawingAsXml(sheet) {
        if(!sheet.media) return;
        var d = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">'
        for (var i = 0; i < sheet.media.length; i++) {
            d += templateDrawing
            .replace('{colFrom}', sheet.media[i].col)
            .replace('{rowFrom}', sheet.media[i].row)
            .replace('{colTo}', sheet.media[i].colTo)
            .replace('{rowTo}', sheet.media[i].rowTo)
            .replace('{rid}', i+1)
            .replace(/{id}/g, sheet.media[i].id+1)
            .replace(/{imgid}/g, sheet.media[i].id)
            // .replace('{creationId}', '_' + sheet.media[i].id)
            .replace('{creationId}', 'D3D4A1C3-C707-4DA3-94F8-E8120DA9F327')
    ...
    function getDrawingRelsAsXml(sheet) {
        if(!sheet.media) return;
        var d =  '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            +  '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">';
        for (var i = 0; i < sheet.media.length; i++) {
            d += '<Relationship Target="../media/image'+sheet.media[i].id+'.png" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Id="rId'+(i+1)+'"/>'
        }
        d += '</Relationships>';
        return d;
    }
    ...
    function createSheets() {
        var oSheets = {
            sheets: [],
            add: function (name) {
                var sheet = { id: this.sheets.length + 1, rId: "rId" + (1 + this.sheets.length), name: name, rows: [], columns: [], getColumn: getColumn, set: setSheet, getRow: getRow, getCell: getCell, mergeCells: [], views: [], freezePane: freezePane };
            ...
            toContentType: function () {
                ...
                s = s + '<Default Extension="png" ContentType="image/png"/>';
                ...
                for (var i = 0; i < this.sheets.length; i++) {
                    s = s + '<Override ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml" PartName="/xl/worksheets/sheet' + (i+1) + '.xml"/>';
                    if(this.sheets[i] && this.sheets[i].media) {
                        s = s + '<Override ContentType="application/vnd.openxmlformats-officedocument.drawing+xml" PartName="/xl/drawings/drawing' + (i+1) + '.xml" />';
                    }
                }
                return s + '</Types>';
            },
            fileData: function (xl) {
                for (var i = 0; i < this.sheets.length; i++) {
                    ...
                    if(this.sheets[i].media) {
                        xl.file('worksheets/_rels/sheet' + (i + 1) + '.xml.rels', getSheetRelsAsXml(this.sheets[i], i+1));
                        xl.file('drawings/drawing' + (i + 1) + '.xml', getDrawingAsXml(this.sheets[i]));
                        xl.file('drawings/_rels/drawing' + (i + 1) + '.xml.rels', getDrawingRelsAsXml(this.sheets[i]));
                        this.sheets[i].media.forEach(img => {
                            xl.file('media/image' + img.id + '.png', img.data, {base64: true});
                        });
                    }
                }
            }
    ...
    function generateColums(columns) {
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            if (c) {
                ...
                if (c.wt == "auto") s = s + ' width="25" bestFit="1" customWidth="1" '; else if (c.wt) s = s + ' width="' + c.wt + '" customWidth="1" ';
    ...
    $JExcel.new = function (defaultFont) {
        ...
        var imagesCount = 0;
        /**
         * Add a image to the generated zip file.
         * @param   {String} data  The file data, base64 encoded
         */
        excel.addImage = function (s=0, col, row, colTo, rowTo, data) {
            if (!isObject(s))s=sheets.get(s);
            if (!s.media) s.media = [];
            s.media.push({id: ++imagesCount, data: data, sheet: s, col: col, row: row, colTo: colTo, rowTo: rowTo});
        }
